...
<form class="variations_form cart" method="post" enctype='multipart/form-data' data-product_id="<?php echo absint( $product->get_id() ); ?>" data-product_variations="<?php echo htmlspecialchars( json_encode( $available_variations ) ) ?>">
	...
	<?php if ( empty( $available_variations ) && false !== $available_variations ) : ?>
		<button type="submit" class="single_add_to_cart_button btn btn-small alt" disabled="disabled">
			<?php echo esc_html_e( 'This product is currently out of stock and unavailable.', 'argenta' ); ?>
		</button>
	<?php else : ?>
			...
			<tbody>
				<tr>
					<?php foreach ( $attributes as $attribute_name => $options ) : ?>
						...
						<?php
							echo end( $attribute_keys ) === $attribute_name ? apply_filters( 'woocommerce_reset_variations_link', '<td class="reset"><a class="reset_variations" href="#"><i class="ion-ios-close-outline"></i><span>' . esc_html__( 'Reset', 'argenta' ) . '</span></a></td>' ) : '';	
						?>
					<?php endforeach;?>
				</tr>
			</tbody>
