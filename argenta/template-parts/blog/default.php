...
?>
<article class="blog-item<?php if ($argenta_post['boxed']) {
    echo ' blog-item-boxed';
}
if (in_array('sticky', get_post_class('', $argenta_post['post_id']))) {
    echo ' sticky';
    ...
    <div class="blog-item-image-wrap">
        ...
        elseif ($argenta_post['media']['audio']) :
            // audio
            echo wp_kses($argenta_post['media']['audio'], 'post');
        elseif ($argenta_post['media']['gallery']) :
            //gallery
            echo wp_kses($argenta_post['media']['gallery'], 'post');
        elseif ($argenta_post['media']['image']) :
            // simple link image ?>
            <a href="<?php echo esc_url($argenta_post['url']); ?>">
                <img class="full-width" src="<?php echo esc_url($argenta_post['media']['image']); ?>"
                     alt="<?php echo esc_attr($argenta_post['title']); ?>">
            </a>
        <?php endif; ?>
    </div>
    ...
    <div class="blog-item-content">
            ...
            <div class="category">
                <?php foreach ($argenta_post['categories'] as $_category) : ?>
                    <a class="brand-border-color brand-color"
                       href="<?php echo esc_url(get_category_link($_category->cat_ID)); ?>">
        ...
        <?php if (!$argenta_post['media']['blockquote']) : ?>
            ...
            <h3 class="title text-left">
                <?php if (in_array('sticky', get_post_class('', $argenta_post['post_id']))) {
                    echo '<span class="ion-pin"></span>';
                } ?>
                <a href="<?php echo esc_url($argenta_post['url']); ?>"><?php echo esc_html($argenta_post['title']); ?></a>
            </h3>
